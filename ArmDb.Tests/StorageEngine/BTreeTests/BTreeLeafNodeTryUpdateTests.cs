...
public partial class BTreeLeafNodeTests
{
  ...
  public void TryUpdate_WhenNewDataIsSmaller_SucceedsInPlace()
  {
    ...
    var originalRow = new ArmDb.DataModel.Record(originalKey.Values[0], DataValue.CreateString("Original Long Data"));
    Assert.True(leafNode.TryInsert(originalRow));

    // 2. Create the new row with the SAME key but SHORTER data
    var rowWithUpdate = new ArmDb.DataModel.Record(originalKey.Values[0], DataValue.CreateString("New Short"));
    ...
    ArmDb.DataModel.Record? updatedRow = leafNode.Search(originalKey);
  ...
  public void TryUpdate_WhenNewDataIsLarger_WithEnoughSpace_SucceedsOutOfPlace()
  {
    ...
    var originalKey = new Key([DataValue.CreateInteger(100)]);
    var originalRow = new ArmDb.DataModel.Record(originalKey.Values[0], DataValue.CreateString("Small"));
    Assert.True(leafNode.TryInsert(originalRow));

    // 2. Create the new row with the SAME key but LARGER data
    var rowWithUpdate = new ArmDb.DataModel.Record(originalKey.Values[0], DataValue.CreateString("This is now much larger data"));
  ...
  public void TryUpdate_WhenNewDataIsLarger_WithoutEnoughSpace_ReturnsFalse()
  {
    ...
    var smallRow = new ArmDb.DataModel.Record(DataValue.CreateInteger(100), DataValue.CreateString("small"));
    ...
    var largeRow = new ArmDb.DataModel.Record(DataValue.CreateInteger(200), DataValue.CreateString(Encoding.UTF8.GetString(largeData)));
    ...
    var rowWithUpdate = new ArmDb.DataModel.Record(DataValue.CreateInteger(100), DataValue.CreateString("This update is way too large to fit in the 50 bytes of free space"));
  ...
  public void TryUpdate_WhenRecordNotFound_ThrowsRecordNotFoundException()
  {
    ...
    var existingRow = new ArmDb.DataModel.Record(DataValue.CreateInteger(100), DataValue.CreateString("Existing Data"));
    Assert.True(leafNode.TryInsert(existingRow));

    // Create an update for a key that does not exist
    var rowWithUpdate = new ArmDb.DataModel.Record(DataValue.CreateInteger(999), DataValue.CreateString("Non-existent record"));
